...
function getHolidayInfo() {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(
        "calendarific sends info", data
      );

      console.log('show me the data', data);
      // maybe show data.response.holidays[0].date.iso
      document.querySelector("h2").innerText = data.response.holidays[0].description;
      document.querySelector("p").innerText = new Date (data.response.holidays[0].date.iso).toLocaleDateString();
      // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toLocaleTimeString 
      document.querySelector("h4").innerText = data.response.holidays[0].primary_type;
    })
    .catch((err) => {
